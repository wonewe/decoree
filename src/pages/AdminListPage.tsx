...
export default function AdminListPage() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter((event) => {
      const target = `${event.title} ${event.description} ${event.id} ${event.location}`.toLowerCase();
      return target.includes(normalizedQuery);
    });
  ...
  const filteredPhrases = useMemo(() => {
    ...
    return phrases.filter((phrase) => {
      const target = `${phrase.korean} ${phrase.translation} ${phrase.id}`.toLowerCase();
      return target.includes(normalizedQuery);
    });
  ...
  const filteredPopups = useMemo(() => {
    ...
    return popups.filter((popup) => {
      const target = `${popup.title} ${popup.brand} ${popup.description} ${popup.location} ${popup.id}`.toLowerCase();
      return target.includes(normalizedQuery);
    });
  ...
  const clearSelection = (section: ContentType) => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`선택한 ${ids.length}개의 콘텐츠를 정말 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.`)) {
      return;
    }
    ...
    try {
      ...
      if (activeSection === "trends") {
        await Promise.all(ids.map((id) => deleteTrendReport(id)));
        setTrends((prev) => prev.filter((item) => !ids.includes(item.id)));
      } else if (activeSection === "events") {
        await Promise.all(ids.map((id) => deleteEvent(id)));
        setEvents((prev) => prev.filter((item) => !ids.includes(item.id)));
      } else if (activeSection === "phrases") {
        await Promise.all(ids.map((id) => deletePhrase(id)));
        setPhrases((prev) => prev.filter((item) => !ids.includes(item.id)));
