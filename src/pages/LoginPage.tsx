    ...
    <section className="section-container">
      <div className="mx-auto max-w-lg space-y-6 rounded-3xl border border-[var(--border)] bg-[var(--paper)] p-8 shadow-xl">
        <header className="space-y-2 text-center">
          ...
          <h1 className="text-3xl font-bold text-[var(--ink)]">{t("auth.title")}</h1>
          <p className="text-sm text-[var(--ink-muted)]">{t("auth.subtitle")}</p>
          {firebaseError && (
        ...
        <div className="text-center text-sm text-[var(--ink-muted)]">
          {t("auth.noAccount")}{" "}
          <Link to="/signup" className="font-semibold text-[var(--ink)] underline-offset-4 hover:underline">
            {t("auth.goToSignup")}
          </Link>
        </div>
